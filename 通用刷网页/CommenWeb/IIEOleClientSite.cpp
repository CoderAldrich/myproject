...
BEGIN_INTERFACE_MAP(IIEOleControlSite, COleControlSite)
	...
	//INTERFACE_PART(IIEOleControlSite, IID_IDispatch, Dispatch)
END_INTERFACE_MAP()
...
ULONG FAR EXPORT IIEOleControlSite::XDispatch::AddRef()   
{   
	METHOD_PROLOGUE(IIEOleControlSite, Dispatch)   
		return pThis->ExternalAddRef();   
}
ULONG FAR EXPORT IIEOleControlSite::XDispatch::Release()   
{
	METHOD_PROLOGUE(IIEOleControlSite, Dispatch)   
		return pThis->ExternalRelease();   
}   
HRESULT FAR EXPORT IIEOleControlSite::XDispatch::QueryInterface(REFIID riid, void **ppvObj)   
{   
	METHOD_PROLOGUE(IIEOleControlSite, Dispatch)   
		HRESULT hr = (HRESULT)pThis->ExternalQueryInterface(&riid, ppvObj);   
	return hr;   
...
HRESULT IIEOleControlSite::XDispatch::GetTypeInfoCount( 
							__RPC__out UINT *pctinfo)
{
	return E_NOTIMPL;
}

HRESULT IIEOleControlSite::XDispatch::GetTypeInfo( 
					   UINT iTInfo,
					   LCID lcid,
...
HRESULT IIEOleControlSite::XDispatch::GetIDsOfNames( 
...
#include <MsHtmdid.h>
HRESULT IIEOleControlSite::XDispatch::Invoke( 
				  DISPID dispIdMember,
				  REFIID riid,
				  LCID lcid,
				  WORD wFlags,
				  ...
				  UINT *puArgErr)
{
 	...
 	case DISPID_AMBIENT_DLCONTROL:
 		pVarResult->vt = VT_I4;
 		pVarResult->lVal = /*DLCTL_DLIMAGES | DLCTL_VIDEOS |*/ /*DLCTL_NO_SCRIPTS |*/ DLCTL_NO_RUNACTIVEXCTLS|DLCTL_SILENT|DLCTL_NO_DLACTIVEXCTLS;
 		return S_OK;
 		break;
...
// 	GetTempFileNameW(szTempPath,L"fdl", 0,szTempFile);
// 
// 	URLDownloadToFileW(NULL,strFileUrl,szTempFile,0,NULL);

	return pThis->m_pView->OnDownloadFile(strFileUrl);
}