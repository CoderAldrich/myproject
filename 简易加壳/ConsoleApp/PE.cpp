...
void CPE::InitValue()
{
	...
	m_dwFileAlign		= 0;
	m_dwMemAlign		= 0;
	ZeroMemory(&m_PERelocDir,sizeof(m_PERelocDir));
	ZeroMemory(&m_PEImportDir,sizeof(m_PEImportDir));
	m_IATSectionBase	= 0;
	m_IATSectionSize	= 0;
	...
	LPBYTE& pFinalBuf, DWORD& pFinalBufSize, BYTE btKey)
{
	...
	PIMAGE_SECTION_HEADER AddSectionHeader =
		&pSectionHeader[pNtHeader->FileHeader.NumberOfSections - 1];
	memcpy_s(AddSectionHeader->Name, 8, ".vmprt", 7);
	...
	while (pPEImport->Name)
	{
		...
		while (pIAT->u1.AddressOfData)
		{
			...
			else
			{
				...
				//清除函数名和函数序号
				memset(pstcFunName, 0, strlen((const char *)(pstcFunName->Name)) + sizeof(WORD));
			}
	...
	while (pPEImport->Name)
	{
		...
		while (pIAT->u1.AddressOfData)
		{
			...
			else
			{
				...
				DWORD dwFunNameRVA = pIAT->u1.AddressOfData;
				PIMAGE_IMPORT_BY_NAME pstcFunName = (PIMAGE_IMPORT_BY_NAME)(m_pFileBuf + dwFunNameRVA);
				dwSizeOfFunBuf += (strlen((const char *)(pstcFunName->Name)) + 1);
			}
			pIAT++;
		}
		pPEImport++;
	...
	while (pPEImport->Name)
	{
		...
		while (pIAT->u1.AddressOfData)
		{
			...
			else
			{
				...
				DWORD dwFunNameRVA = pIAT->u1.AddressOfData;
				PIMAGE_IMPORT_BY_NAME pstcFunName = (PIMAGE_IMPORT_BY_NAME)(m_pFileBuf + dwFunNameRVA);
				memcpy_s((PCHAR)m_pFunNameBuf + TempFunRVA, strlen((const char *)(pstcFunName->Name)) + 1,
					pstcFunName->Name, strlen((const char *)(pstcFunName->Name)) + 1);

				m_pMyImport[TempNumOfFuns].m_dwFunNameRVA = TempFunRVA;
				m_pMyImport[TempNumOfFuns].m_dwModNameRVA = TempModRVA;
				TempFunRVA += (strlen((const char *)(pstcFunName->Name)) + 1);
			}
			TempNumOfFuns++;
			pIAT++;
		}
