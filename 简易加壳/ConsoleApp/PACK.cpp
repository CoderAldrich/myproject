...
// 返 回	值:	BOOL
//************************************************************
BOOL CPACK::Pack(LPCWSTR pszFilePath,LPCWSTR pszOutputFilePath , BOOL bSelect[5], CHAR MachineCode[16], BYTE btKey)
{
	//1.读取文件PE文件信息并保存
	CPE objPE;
	if (objPE.InitPE(pszFilePath) == FALSE)
		return FALSE;
	...
	objPE.MergeBuf(objPE.m_pFileBuf, objPE.m_dwImageSize,
		...
		pFinalBuf, dwFinalBufSize,btKey);

	//5.保存文件（处理完成的缓冲区）
	SaveFinalFile(pFinalBuf, dwFinalBufSize, pszOutputFilePath);
...
// 返 回	值:	BOOL
//************************************************************
BOOL CPACK::SaveFinalFile(LPBYTE pFinalBuf, DWORD pFinalBufSize, LPCWSTR pszOutputFilePath)
{
