...
VOID CUserHandle::EnumHandles( TypeHandleEnumCallback pCallback , PVOID pParam )
{

	PHANDLE_ARRARY pTempHandleArray = m_pHandleArray;
	int nGroupIndex = 0;

	while (pTempHandleArray)
	{
		...
		for ( int i=0;i<256;i++ )
		{
			if ( pTempHandleArray->usDataInUse[i] != 0 )
			{
				pCallback(pParam,(HANDLE)MAKELONG(nGroupIndex*256+i+1,pTempHandleArray->usDataInUse[i]));
			}
		}
