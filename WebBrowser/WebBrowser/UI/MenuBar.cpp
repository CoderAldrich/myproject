...
IMPLEMENT_DYNAMIC(C58MenuBar, CToolBar)

BEGIN_MESSAGE_MAP(C58MenuBar, CToolBar)
	ON_WM_CREATE()
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
...
//
int C58MenuBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//
HMENU C58MenuBar::LoadMenu(HMENU hmenu)
{
	UINT iPrevID=(UINT)-1;
	ASSERT(::IsMenu(hmenu));
	ASSERT_VALID(this);
	...
	BOOL bModifyStyle = ModifyStyle(0,/* TBSTYLE_LIST|*/TBSTYLE_TRANSPARENT);
...
//
HMENU C58MenuBar::LoadMenu(LPCSTR lpszMenuName)
{
	return LoadMenu(::LoadMenuA(AfxGetResourceHandle(), lpszMenuName));
}

//////////////////
// Set menu bar font from current system menu font
//
//void C58MenuBar::UpdateFont()
//{
//	static CFont font;
...
//
void C58MenuBar::OnUpdateMenuButton(CCmdUI* pCmdUI)
{
	ASSERT_VALID(this);
...
//
void C58MenuBar::SetTrackingState(TRACKINGSTATE iState, int iButton)
{
	ASSERT_VALID(this);
...
//
void C58MenuBar::ToggleTrackButtonMode()
{
	ASSERT_VALID(this);
...
//
int C58MenuBar::GetNextOrPrevButton(int iButton, BOOL bPrev)
{
	ASSERT_VALID(this);
	if (bPrev)
	{
...
//
int C58MenuBar::HitTest(CPoint p) const
{
...
//
void C58MenuBar::OnLButtonDown(UINT nFlags, CPoint pt)
{
	try{
	ASSERT_VALID(this);
...
//
void C58MenuBar::OnMouseMove(UINT nFlags, CPoint pt)
{
	ASSERT_VALID(this);
...
//
void C58MenuBar::OnMenuSelect(HMENU hmenu, UINT iItem)
{
...
//
static C58MenuBar*	g_pMenuBar = NULL;
...
//
LRESULT CALLBACK
C58MenuBar::MenuInputFilter(int code, WPARAM wp, LPARAM lp)
{
...
//
BOOL C58MenuBar::OnMenuInput(MSG& m)
{
	ASSERT_VALID(this);
...
//
void C58MenuBar::CancelMenuAndTrackNewOne(int iNewPopup)
{
	ASSERT_VALID(this);
...
//
void C58MenuBar::TrackPopup(int iButton)
{
	ASSERT_VALID(this);
	ASSERT(m_hmenu);
	...
	{					 
		...
		if(mainframe)
		{
			...
			if( iButton == 3)
			{
				//mainframe->BuildFavoriteMenu(&submenu);
				returncmd = submenu.TrackPopupMenu(TPM_LEFTALIGN|TPM_LEFTBUTTON|TPM_VERTICAL|TPM_RETURNCMD,pt.x, pt.y, GetParentFrame());

			}
			else
			{
				returncmd = submenu.TrackPopupMenu(TPM_LEFTALIGN|TPM_LEFTBUTTON|TPM_VERTICAL,pt.x, pt.y, GetParentFrame());
				int a=0;
			}
			//mainframe->DynamicMenuCmd(returncmd,iButton);
		}
...
//
CPoint C58MenuBar::ComputeMenuTrackPoint(const CRect& rcButn, TPMPARAMS& tpm)
{
...
//
BOOL C58MenuBar::TranslateFrameMessage(MSG* pMsg)
{
	ASSERT_VALID(this);
	ASSERT(pMsg);
...
#ifdef _DEBUG
void C58MenuBar::AssertValid() const
{
}

void C58MenuBar::Dump(CDumpContext& dc) const
{
}
#endif

BOOL C58MenuBar::MapAccelerator(TCHAR ch, UINT & nID)
{
	return SendMessage(TB_MAPACCELERATOR, (WPARAM)ch, (LPARAM)&nID); 
}

LRESULT C58MenuBar::OnNcHitTest(CPoint point) 
{
